...
export function handle_user_ratio(ratio) {
  ...
  const stroke_color = audit_ratio < 0.5 ? "#fff" : audit_ratio < 1.2 ? "#764ba2" : "#fff";

  const svg = createSvgElement("svg", { width: 100, height: 100, viewBox: "0 0 100 100" }, ["ratio-svg"]);
  const circle = createSvgElement("circle", { cx: 50, cy: 50, r: radius, fill: color, stroke: stroke_color, "stroke-width": 7 });
  const text = createSvgElement("text", { x: 50, y: 55, "text-anchor": "middle", "dominant-baseline": "middle" }, ["ratio-text"]);
